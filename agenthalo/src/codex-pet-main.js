...
function createCodexPetMain(options = {}) {
  ...
  function getActiveThemeId() {
    ...
    return activeTheme ? activeTheme._id : (settingsController.get("theme") || "pikachu");
  }
  ...
  async function refreshFromSettings() {
    ...
    if (summaryHasActiveCodexPetOrphan(summary, activeId)) {
      const result = await settingsController.applyCommand("setThemeSelection", { themeId: "pikachu" });
      if (!result || result.status !== "ok") {
        return {
          status: "error",
          message: (result && result.message) || "failed to switch active orphan Codex Pet theme back to pikachu",
          summary,
        };
      }
      switchedToFallback = true;
      const cleanup = syncThemes("pikachu");
