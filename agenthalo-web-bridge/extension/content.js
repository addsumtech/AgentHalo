...
//
// Each conversation route gets its own session. The background worker adds
// Chrome's tab ID, so duplicated tabs cannot share a desktop task card.
(() => {
  ...
  let sessionId = `chat-${sessionToken()}-${conversationUrl()}`;
  let currentUrl = conversationUrl();

  function conversationUrl() {
    return clawdConversationUrl(location.origin + location.pathname + (location.search || ""));
  }

  function payload(state, event, extra = {}) {
    const t = (site.title() || "").slice(0, 120);
    return Object.assign({
      agent_id: (CFG.agents && CFG.agents[site.key]) || "",
  ...
  function send(body) {
    ...
    return new Promise((resolve) => {
      try {
        chrome.runtime.sendMessage({ type: "clawd-state", siteKey: site.key, body }, (res) => {
  ...
  function check() {
    ...
    const nextUrl = conversationUrl();
    if (nextUrl !== currentUrl || site.skip()) {
      if (phase !== "idle") send(payload("idle", "SessionEnd"));
      phase = "idle";
      lastTitle = null;
      currentUrl = nextUrl;
      sessionId = `chat-${sessionToken()}-${currentUrl}`;
    }
    if (site.skip()) return;
  ...
  globalThis.__clawdWebBridge = {
    get phase() { return phase; },
    get sessionId() { return sessionId; },
    get agentId() { return (CFG.agents && CFG.agents[site.key]) || ""; },
    siteKey: site.key,
    check,
  };
})();
